...
#define FT5X46_INT_NAME "ft5x46_gpio_intr"
#define FT5X46_RESET_NAME "ft5x46_gpio_reset"
...
#ifndef CONFIG_PM_SLEEP
static ssize_t ft5x46_power_hal_suspend_store(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
	struct ft5x46_data *ft5x46 = dev_get_drvdata(dev);
...
#ifndef CONFIG_PM_SLEEP
static DEVICE_ATTR(power_HAL_suspend, 0222, NULL, ft5x46_power_hal_suspend_store);
#endif
...
#ifndef CONFIG_PM_SLEEP
static DEVICE_ATTR(power_HAL_suspend, 0200, NULL, ft5x46_power_hal_suspend_store);
#endif
#endif
			...
			unsigned long event, void *data)
{
	...
	if (blank && event == TP_POWER_STATE_EVENT && ft_data) {
		if (*blank == TP_POWER_RESUME) {
			dev_dbg(ft_data->dev, "##### UNBLANK SCREEN #####\n");
			ft5x46_input_enable(ft_data->input);
		} else if (*blank == TP_POWER_SUSPEND) {
			dev_dbg(ft_data->dev, "##### BLANK SCREEN #####\n");
			ft5x46_input_disable(ft_data->input);
		}
	}
...
static void configure_sleep(struct ft5x46_data *data)
{
	int ret;
...
#ifdef CONFIG_OF
static void ft5x46__dump_value(struct device *dev,
...
#else
static int ft5x46_parse_config(struct device *dev,
			struct ft5x46_ts_platform_data *pdata)
{
	...
	pdata->z_max = 255;
	pdata->w_max = 200;
	pdata->landing_jiffies = 1000;
	pdata->landing_threshold = 16;
	pdata->staying_threshold = 8;
	pdata->raw_min = 7000;
	pdata->raw_max = 10000;
	pdata->ui.delay_55 = 2;
	pdata->ui.delay_aa = 2;
	pdata->ui.upgrade_id_1 = 0x58;
	pdata->ui.upgrade_id_2 = 0x2C;
	pdata->ui.delay_readid = 20;
	pdata->cfg_size = 2;
	pdata->firmware = kmalloc(sizeof(struct ft5x46_firmware_data) * (pdata->cfg_size + 1),
				GFP_KERNEL);
	if (pdata->firmware == NULL)
		return -ENOMEM;
	pdata->testdata = kmalloc(sizeof(struct ft5x46_test_data) * pdata->cfg_size, GFP_KERNEL);
	if (pdata->testdata == NULL)
		return -ENOMEM;

	pdata->firmware[0].chip = 0x58;
	pdata->firmware[0].vendor = 0x55;
	pdata->firmware[0].fwname = "ft5626_laibao.fw";
	pdata->firmware[0].size = 0;
	pdata->testdata[0].tx_num = 24;
	pdata->testdata[0].rx_num = 13;

	pdata->firmware[1].chip = 0x58;
	pdata->firmware[1].vendor = 0x3b;
	pdata->firmware[1].fwname = "ft5626_biel.fw";
	pdata->firmware[1].size = 0;
	pdata->testdata[1].tx_num = 24;
	pdata->testdata[1].rx_num = 13;
				...
				const struct ft5x46_bus_ops *bops)
{
	...
	index = 0;
	gpio = devm_gpiod_get_index(dev, FT5X46_RESET_NAME, index);
	if (!IS_ERR(gpio)) {
		pdata->reset_gpio = desc_to_gpio(gpio);
		gpiod_direction_output(gpio_to_desc(pdata->reset_gpio), 0);
		ndelay(100);
		gpiod_direction_output(gpio_to_desc(pdata->reset_gpio),	1);
	} else {
		pdata->reset_gpio = -1;
	...
	index = 2;
	gpio = devm_gpiod_get_index(dev, FT5X46_INT_NAME, index);
	if (!IS_ERR(gpio)) {
		pdata->irq_gpio = desc_to_gpio(gpio);
		gpiod_to_irq(gpio_to_desc(pdata->irq_gpio));
	...
	error = ft5x46_parse_config(dev, pdata);
	if (error)
		goto free_irq_gpio;
...
#ifdef CONFIG_FB
	configure_sleep(ft5x46);
#endif

#ifndef CONFIG_PM_SLEEP
	register_power_hal_suspend_device(ft5x46->dev);
#endif
